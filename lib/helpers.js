  ...
  while (attempts < 50) {
    try {
      ...
      if (attempts >= 50) {
        ...
        await notifyWebhook(
          `An HTTP error has occurred (${response.status}) while making a web request. Please check the logs for further details.`
        );
        throw new Error(
          `HTTP error! Status: ${response.status} - ${
            "errors" in data ? data.errors[0].message : JSON.stringify(data)
          }`
        );
      }
      await wait(5000);
